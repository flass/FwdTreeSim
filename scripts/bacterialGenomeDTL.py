...
def usage():
	...
	l += ["\t-s  --popsize   int\t\tnumber of species to simulate in the underlying Moran process\t# defaults to 100."]
	l += ["\t-g  --ngen      int\t\tnumber of generation for which the evolution is simulated\t# defaults to 1000."]
	
	l += ["  __Gene/Locus layer__:"]
	l += ["\t-n  --ngenes    int\t\tnumber of gene families to simulate in the pangenome\t# defaults to 10."]
	l += ["\t-p  --profiles  path\t\tJSON file containing the (multiple) evolutionary profiles for simulated gene families, ", \
	      crindent+"and their respective weights (sampling probability if float or expected number of gene families if integer).", \
	      crindent+"See FwdTreeSim/example/DTLprofiles.json for file format example. Example pangenome structure (with 20% core, 20% accessory,", \
	      crindent+" 60% orfan gene families) can be generated by replacing path by an empty string ''. "]
	l += ["\t-r  --dtlrates  float[,float[,float]]\tglobal rates of Duplication, Transfer and Loss for ALL simulated gene families.", \
	      crindent+"# defaults to D=0.0001 T=D, L=D+T ; overriden by providing gene family profiles."]
	l += ["\t-f  --rootfreq  float\t\tglobal presence probability for ALL the gene families at the root of the simulation of the species population.", \
	      crindent+"# defaults to 0.5 ; overriden by providing gene family profiles."]
	...
	l += ["\t-c  --connect.all.trees float\tReturn the multiple gene trees from a gene family population as one, connecting them at their root.", \
	      crindent+"Length of branches of the star root is given by the float argument # defaults to 0 (active) overrides -l option)."]
	return '\n'.join(l)
...
def main():
	...
	popsize = dopt.get('-s', dopt.get('--popsize', 100))
	ngen = dopt.get('-g', dopt.get('--ngen', 1000))

	nfprofiles = dopt.get('-p', dopt.get('--profiles'))
	# define the evolution rates and original frequencies of gene families
	if nfprofiles!=None:
		if nfprofiles=='':
			dtlprof = IOsimul.MetaSimulProfile(profiles=[(n, IOsimul.DTLSimulProfile(type=t)) for n,t in ((2, 'core'), (2, 'accessory-slow'), (6, 'orfan-fast'))])
		else:
		...
		rootfreq = dopt.get('-f', dopt['--rootfreq'], 0.5)
		...
		dtlprof = IOsimul.MetaSimulProfile([(1, IOsimul.DTLSimulProfile({'rootfreq':rootfreq, 'rateschedule': {0:{'rdup':dtlrates[0], 'rtrans':dtlrates[1], 'rloss':dtlrates[2]}}} ))])
	...
	nlargegenetrees = int(dopt.get('-l', dopt.get('--sample.larger.trees', 5)))
	lentoroot = float(dopt.get('-c', dopt.get('--connect.all.trees', -1)))
	samplextant = int(dopt.get('-e', dopt.get('--sample.extant.species')))
	...
	if lentoroot>=0:
		...
		extconrt = moransim.get_extanttrees(compute=True, use_copy=True, connecttrees=lentoroot)
		extconrt.write_newick("%s/reftrees/connected.reftree_extant.nwk"%(outdir))
	else:
		lextrt = moransim.get_extanttrees(compute=True, use_copy=True)
	...
	for k in range(ngenes):
		...
		if lentoroot>=0:
			...
			congt.write_newick("%s/genetrees/simul.%d.gt_full.nwk"%(outdir, k))
