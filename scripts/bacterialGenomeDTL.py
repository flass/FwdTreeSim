...
def usage():
	...
	l += ["\t-p  --profiles  path\t\tJSON file containing the (multiple) evolutionary profiles for simulated gene families, ", \
	      crindent+"and their respective weights (sampling probability if float or expected number of gene families if integer numbers).", \
	...
	l += ["\t-n  --ngenes    int\t\tnumber of gene families to simulate in the pangenome\t# defaults to 10; overriden by providing gene family profiles."]
	l += ["\t-r  --dtlrates  float[,float[,float]]\tglobal rates of Duplication, Transfer and Loss for ALL simulated gene families.", \
	      crindent+"# defaults to D=0.001 T=D, L=D+T; overriden by providing gene family profiles."]
	...
	l += ["\t-c  --connect.all.trees float\tIn addition, return the multiple gene trees from a gene family population as one, connecting them at their root.", \
	      crindent+"Length of branches of the star root is given by the float argument, negative value turns it off # defaults to 0 (active))."]
	return '\n'.join(l)
